...
class CustomGaugeCard extends LitElement {
  ...
  _renderGauge(gradient, min, max, name, entity, state, unit) {
    ...
    //const angle = isNaN(value) ? -90 : this._valueToAngle(value, min, max);
    const showNeedle = !isNaN(value);
    const angle = showNeedle ? this._valueToAngle(value, min, max) : -90;
    ...
    const needle = svg`
    <g transform="translate(${cx}, ${cy}) rotate(${angle})" style="transition: transform 0.6s ease;">
      <polygon points="${needlePoints}" fill="${cfg.needle_color}" opacity="${
      showNeedle ? cfg.needle_opacity : 0
    }" />
    </g>
  `;
    const extraNeedles = (cfg.needles ?? [])
      .map((n) => {
        ...
        const label = this._polarToCartesian(
          cx,
          cy,
          radius + cfg.strokeWidth / 2 + 18,
          a
        );
    ...
    const needleInfoTexts = (cfg.needles ?? []).map((n, i) => {
      ...
      const xOffset = -10;
      return svg`
        <text x="${xOffset}" y="${yOffset}" font-size="${
        cfg.tick_font_size
    ...
    for (let i = 0; i <= cfg.ticks_count; i++) {
      ...
      const inner = this._polarToCartesian(
        cx,
        cy,
        radius - cfg.strokeWidth * cfg.tick_stroke_inner,
        tickAngle
      );
      const outer = this._polarToCartesian(
        cx,
        cy,
        radius + cfg.strokeWidth * cfg.tick_stroke_outer,
        tickAngle
      );
      const label = this._polarToCartesian(
        cx,
        cy,
        radius - cfg.strokeWidth / 2 - 12,
        tickAngle
      );
      ...
      if (i !== 0 && i !== cfg.ticks_count) {
        ticks.push(
          svg`<line x1="${inner.x}" y1="${inner.y}" x2="${outer.x}" y2="${outer.y}" stroke="${cfg.tick_color}" stroke-width="${cfg.tick_width}" />`
        );
      }
      ticks.push(
        svg`<text x="${label.x}" y="${label.y}" text-anchor="middle" font-size="${cfg.tick_font_size}" dominant-baseline="middle">${labelText}</text>`
      );
    }

    const statMarkers = ["min", "max", "avg"]
      .map((key) => {
        ...
        return svg`
        <g>
        <line x1="${inner.x}" y1="${inner.y}" x2="${outer.x}" y2="${
          outer.y
        }" stroke="${col}" stroke-width="${cfg.markers_width}" />
          <text x="${label.x}" y="${label.y}" text-anchor="middle" font-size="${
          cfg.tick_font_size
        }" fill="${col}" dominant-baseline="middle">${formatNumber(
          val,
          cfg.stat_decimals
        )}</text>
        </g>
    ...
    const paths = ranges.map((range) => {
      ...
      return svg`<path d="${this._describeArc(
        cx,
        cy,
        radius,
        start,
        end
      )}" fill="none" stroke="${range.color}" stroke-width="${
        cfg.strokeWidth
      }" />`;
    });

    // Add fallback arc in gray if no gradient ranges exist
    if (paths.length === 0) {
      paths.push(svg`
    <path d="${this._describeArc(cx, cy, radius, -90, 90)}"
          fill="none"
          stroke="#ccc"
  ...
  `);
    ...
    return html`
      <svg
        viewBox="0 0 240 180"
        style="position: absolute; width: 100%; height: 100%; top: 7%; left: 0;"
      >
        ${paths} ${ticks} ${statMarkers} ${extraNeedles} ${needleInfoTexts}
        ${needle}
        <text
          x="${cx}"
          y="${cfg.titel_font_size * 0.75}"
          text-anchor="middle"
          font-size="${cfg.titel_font_size}"
          font-weight="bold"
        >
          ${name}
        </text>
        ...
        <text
          ...
          y="${leftPos.y}"
          text-anchor="middle"
          font-size="${cfg.rltext_font_size}"
        >
          ${cfg.leftText}
        </text>
        <text
